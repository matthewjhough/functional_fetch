...
/***/ (function(module, exports, __webpack_require__) {
	...
	*/
	...
	var results = function results(options, url) {
	  return pipe(createReq, mapEntries, buildQuery, processURI, _fetchFill2.default, resolv, transform)(options, url);
	};

	/*
	    Accept request params, and convert options object to API format
	*/
	var createReq = function createReq(_ref, url) {
	  var colors = _ref.colors,
	      page = _ref.page;

	  var options = { limit: PAGE_COUNT };
	  if (colors) options.colors = colors;
	  if (page) options.offset = page ? page * PAGE_COUNT : 0;
	  return {
	    options: options,
	    url: url
	  };
	...
	*/
	var mapEntries = function mapEntries(_ref2) {
	  var options = _ref2.options,
	      url = _ref2.url;
	  return Object.entries(options).map(function (e) {
	    return [resolveColors(e)];
	  }).reduce(format).map(function (i) {
	    return (0, _urijs2.default)(url).addSearch(i[0], "" + i[1]);
	  });
	};
	...
	*/
	var resolveColors = function resolveColors(_ref3) {
	...
	*/
	var buildQuery = function buildQuery(entries) {
	  var built = entries.reduce(function (acc, val) {
	    return Object.assign(acc, joinQueries(acc, val));
	  });

	  console.log("QUERY BUILT::::::::::::", built);

	  return built;
	};

	var processURI = function processURI(_ref5) {
	  var _ref5$_parts = _ref5._parts,
	      path = _ref5$_parts.path,
	      port = _ref5$_parts.port,
	      hostname = _ref5$_parts.hostname,
	      protocol = _ref5$_parts.protocol,
	      query = _ref5$_parts.query;

	  var currentURL = protocol + "://" + hostname + ":" + port + path + "?" + query;
	  // This is added for the testing environment so i can retrieve it later.
	  // Normally i'd get this information from the response object later on when needed -
	  // to avoid any global mutable variables.
	  window.currentURL = currentURL;
	  console.log("PARTS PARAMS RECIEVED :::::::::::::::", path, port, hostname, protocol, query);
	...
	*/
	var joinQueries = function joinQueries(a, v) {
	  var obj = v;
	  var joined = a._parts.query + "&" + v._parts.query;
	...
	*/
	var resolv = function resolv(res) {
	...
	*/
	var transform = function transform(res) {
	  return res.then(function (res) {
	    var _URI$parseQuery = _urijs2.default.parseQuery((0, _urijs2.default)(res.url)._parts.query),
	        _URI$parseQuery$offse = _URI$parseQuery.offset,
	        offset = _URI$parseQuery$offse === undefined ? 0 : _URI$parseQuery$offse;

	    var prevPage = +offset / PAGE_COUNT - 1;
	    // Send request to check for next page.
	    return (0, _fetchFill2.default)(processURI((0, _urijs2.default)(window.currentURL).removeSearch("offset").addSearch("offset", +offset + 10))).then(function (next) {
	      // Read next page results
	      return next.json().then(function (nextResults) {
	        // Check if next page exists, calculate next page
	        var isNext = nextResults.length > 0;
	        var calcNext = +offset / PAGE_COUNT + 1;
	        var nextPage = isNext ? calcNext : null;
	        return promiseWrapper(
	        // Read original request results, and format data.
	        res.data.then(function (d) {
	          var mapped = d.map(formatObjects);
	          // Return promise containing transformed data.
	          return promiseWrapper({
	            ids: d.map(function (_ref6) {
	              var id = _ref6.id;
	              return id;
	            }),
	            open: mapped.filter(function (item) {
	              return item.disposition === OPEN;
	            }),
	            closedPrimaryCount: mapped.filter(function (_ref7) {
	              var disposition = _ref7.disposition,
	                  isPrimary = _ref7.isPrimary;
	              return disposition !== OPEN && isPrimary;
	            }).length,
	...
	var formatObjects = function formatObjects(_ref8) {
	  var id = _ref8.id,
	      color = _ref8.color,
	      disposition = _ref8.disposition;
	  return {
	    id: id,
	    color: color,
	...
	var setPrimaryStatus = function setPrimaryStatus(color) {
	...
	var promiseWrapper = function promiseWrapper(data) {
	...
	*/
	...
	var retrieve = function retrieve(options) {
	  var url = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : endpoint;
	  return results(options, url);
	};
	// .then(res => res.json())
	// .catch(err => console.log("err: ", err))
	// .then(res => console.log("To transform: ", res));

	// retrieve({ limit: 10 }, endpoint);
	retrieve({ colors: ["red", "blue", "brown"], page: 1 }).then(console.log);
	// retrieve({ page: 3 });
